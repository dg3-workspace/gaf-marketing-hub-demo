  ...
  return (
    <div className="grid md:grid-cols-2 gap-12 items-center">
      <div className={`space-y-4 ${isReversed ? 'md:order-2' : ''}`}>
        <span className="text-6xl font-extrabold text-gray-200">0{index + 1}</span>
      ...
      <div className={isReversed ? 'md:order-1' : ''}>
          {capability.visualType === 'diagram' ? (
            ...
            <div>
              <VideoPlaceholder 
                label={capability.videos[activeVideoIndex]?.label || 'Video Placeholder'} 
...
export const EndUserExperienceSection: React.FC = () => {
  return (
    <section className="min-h-screen bg-white py-20 px-8 flex items-center">
      <div className="container mx-auto">
        <div className="text-center mb-16">
          <h2 className="text-3xl md:text-4xl font-bold text-brand-gray">Part 1: The End-User Experience</h2>
          <p className="text-lg text-gray-500 mt-4 max-w-3xl mx-auto">
            A guided walkthrough of the platform's core features from the perspective of a contractor and territory manager.
          </p>
        </div>
        <div className="space-y-20 max-w-6xl mx-auto">
          {END_USER_CAPABILITIES.map((capability, index) => (
